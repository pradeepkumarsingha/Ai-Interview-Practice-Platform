...
export default function Sidebar({ collapsed = false, mobileOpen = false, onClose = () => {} }) {
  ...
  const navItems = visibleFeatures.map((feature) => {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed left-0 top-16 z-50 h-[calc(100vh-64px)] w-72 border-r border-slate-200/80 bg-white/95 backdrop-blur-xl transition-transform duration-300 dark:border-white/10 dark:bg-slate-950/95 lg:hidden ${
          mobileOpen ? "translate-x-0" : "-translate-x-full"
        }`}
      >
        <nav className="flex h-full w-full flex-col gap-2 overflow-y-auto p-3 pb-6">
          <div className="mb-2 rounded-2xl bg-gradient-to-br from-indigo-50 to-blue-50 p-4 dark:from-indigo-500/10 dark:to-blue-500/10">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-indigo-600 dark:text-indigo-300">Career OS</p>
            <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">Build, measure, improve.</p>
          </div>
          {navItems}
        </nav>
      </aside>
